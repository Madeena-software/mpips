...
Run the scripts from the repository root. Use `venv/bin/python` directly or activate the virtual environment first.
...
1. **Train the Model**:
   ```bash
   venv/bin/python src/neural_model/train.py
   ```
...
2. **Evaluate and Regenerate Artifacts**:
   ```bash
   venv/bin/python src/neural_model/evaluate.py
   ```
...
4. **Warp the Image**:
   ```bash
   venv/bin/python src/neural_model/warp_image.py --step 4 --iterations 10
   ```
   ...
   ```bash
   venv/bin/python src/neural_model/warp_image.py \
...
5. **Validate Outputs**:
   ```bash
   venv/bin/python src/neural_model/validate_outputs.py
   ```
...
- Generate a no-border crop:
  ```bash
  venv/bin/python src/neural_model/warp_image.py --crop-valid
  ```
  This writes `output/neural_model/calibrated_image_cropped.tiff`.
- Hide borders for visual inspection only:
  ```bash
  venv/bin/python src/neural_model/warp_image.py --border-mode replicate
  ```
