...
## PHASE 2: Generate the `.agents/` Structure
...
* Explain the purpose of the `.agents/` folder as the source of truth for AI agent workflow and project context.
* Provide a directory index.
* Add instructions for AI Agents (e.g., "Start every session by reading `memory/state.md`") and Humans.

#### `Create .agents/history.md`
* Create an append-only log.
* Add the first entry for today: `"Session 1: Bootstrap .agents/ Control Center"` and list the audit actions taken and files created.
* Instruct that to conserve context budget, old sessions should be archived (e.g., moved to `.agents/history/archive_YYYY_Qx.md`) periodically once they exceed 15-20 entries.

#### `Create .agents/memory.json`
...
### 2. State & Memory (`.agents/memory/`)

#### `Create .agents/memory/state.md`
...
#### `Create .agents/rules/project-context.md`
...
#### `Create .agents/rules/[stack-name].md`
...
#### `Create .agents/rules/server-access-constraints.md`
...
#### `Create .agents/rules/testing-pyramid.md`
...
#### `Create .agents/prompts/prompts.md`
