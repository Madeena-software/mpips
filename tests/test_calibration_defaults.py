...
def test_neural_calibration_defaults_to_fixed():
    assert NeuralCalibrationConfig().canvas_mode == "fixed"


def test_neural_calibration_explicit_expanded_remains_supported():
    assert NeuralCalibrationConfig(canvas_mode="expanded").canvas_mode == "expanded"


def test_calibration_cli_defaults_to_fixed(monkeypatch, tmp_path):
    captured = {}
...
def test_calibration_cli_explicit_expanded_remains_supported(monkeypatch, tmp_path):
    captured = {}
    monkeypatch.setenv("MPIPS_ARTIFACT_ROOT", str(tmp_path))
    monkeypatch.setattr(
        run_pipeline, "main", lambda args: captured.setdefault("args", args)
