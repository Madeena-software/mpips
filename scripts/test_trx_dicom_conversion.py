...
def main():
    ...
    raw_path = "research/phantom-gotri-thorax/gotri thorax/TRX_1786615175262_rawimage.tiff"
    ...
    camera_params = {"serialNumber": "DA5234480", "cameraSerial": "DA5234480", "cameraModel": "MV-CH120-10UM"}
    ...
    manifest_content = {
        "examination": {
            "study_description": "THORAX PHANTOM RADIOGRAPH"
        },
        "patient": {
    ...
    with open(rad_npz_path, "rb") as rf, open(gain_npz_path, "rb") as gf:
        files = {
            ...
            "manifest": ("manifest.json", manifest_json.encode("utf-8"), "application/json"),
        }

        start_t = time.time()
        with httpx.Client(timeout=180.0) as client:
            res = client.post(f"{API_URL}/v1/radiographs/dicom", headers=headers, files=files)
