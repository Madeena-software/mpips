...
from mpips.calibration.dotgrid.neural_model.dataset import load_data, save_coordinates
...
from mpips.calibration.dotgrid.paths import default_artifact_path
...
def write_report(
    ...
    lines = [
        ...
        "",
        (
            "This report compares the raw dot-grid extraction, an OpenCV "
            "Brown-Conrady calibration baseline, and the existing neural "
            "compensation result."
        ),
        "",
        "## Calibration Setup",
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"| Metric | Raw | OpenCV | {neural_header} | "
                "OpenCV change vs raw | Neural change vs raw |"
            ),
            "| --- | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{neural_value} | "
            f"{format_pct(row['opencv_reduction_pct'])} | "
    ...
    if not neural_available:
        lines.extend(
            [
                "",
                (
                    "Neural metrics were not computed because the neural model "
                    "file was not found."
                ),
            ]
        )
    ...
    lines.extend(
        [
            ...
            (
                "- With only one calibration view, OpenCV intrinsics and "
                "distortion coefficients can be under-constrained; use them "
                "as baseline diagnostics, not as final production camera "
                "parameters."
            ),
            (
                "- The neural result uses the repository's existing learned "
                "compensation field and can model residual target/lens "
                "deformation beyond the OpenCV polynomial fit."
            ),
            (
                "- Diameter StdDev is computed from same-size metal balls "
                "only; the oversized center nut marker is excluded when "
                "detected."
            ),
            (
                "- Lower values are better for every table metric. SMIA "
                "changes are computed from absolute distortion magnitude."
            ),
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run an OpenCV Brown-Conrady calibration baseline and compare it "
            "with the neural result."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out-dir", default=default_artifact_path("output/opencv_baseline")
    )
