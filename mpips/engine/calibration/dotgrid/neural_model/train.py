...
def train_model(
    ...
    if device == "cuda" and not torch.cuda.is_available():
        raise ValueError("CUDA was requested for training but is not available")
    ...
    diams = diams.to(device)
    metal_ball_mask = torch.as_tensor(
        metal_ball_mask_np, dtype=torch.bool, device=device
    )
    ...
    adaptive_loss = AdaptiveLoss(num_losses=3).to(device)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            print(
                ...
                "Weights: "
                f"{torch.exp(-adaptive_loss.log_vars).detach().cpu().numpy()}"
            )
